...
with open("hoff_jsons/toys.hoff.ru.json") as file:
    ...
    for i in range(len(data["data"]["items"])):
        ...
        res.append({"name":name,"image":data["data"]["items"][i]["image"],"prices":data["data"]["items"][i]["prices"],"discount":data["data"]["items"][i]["discount"], "rating":data["data"]["items"][i]["rating"]})
